...
export class CommitteeDeliberationHomeScreen extends Screen implements IHomeScreen {
  ...
  protected async getResponse(interaction?: TrackedInteraction, args?: RenderArgs): Promise<any> {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        'Here, the committee members can submit their evaluation votes on a Funding Round. Begin by selecting a funding round, by pressing the "Select Funding Round" button below.',
      );
...
class SelectFundingRoundAction extends Action {
  ...
  private async handleSelectFundingRound(interaction: TrackedInteraction): Promise<void> {
    ...
    await (this.screen as CommitteeDeliberationHomeScreen).selectVoteTypeAction.handleOperation(interaction, OPERATION_IDS.SHOW_VOTE_TYPES, {
      fundingRoundId,
    });
  }
...
class SelectVoteTypeAction extends Action {
  ...
  private async handleShowVoteTypes(interaction: TrackedInteraction, args: { fundingRoundId: number }): Promise<void> {
    ...
    const embed = new EmbedBuilder().setColor('#0099ff').setTitle(`You're Deliberating On ${fundingRound.name}`)
    ...
    const addNewVoteButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(this, OPERATION_IDS.SELECT_VOTE_TYPE, 'voteType', 'new', FUNDING_ROUND_ID, fundingRoundId.toString()),
      )
    ...
    const changeVoteButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          OPERATION_IDS.SELECT_VOTE_TYPE,
          'voteType',
          'change',
          FUNDING_ROUND_ID,
          fundingRoundId.toString(),
          VOTING_ARGS.SHOW_UNVOTED,
          'false',
        ),
      )
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(addNewVoteButton, changeVoteButton);
  ...
  private async handleSelectVoteType(interaction: TrackedInteraction): Promise<void> {
    ...
    if (voteType === 'new') {
      await (this.screen as CommitteeDeliberationHomeScreen).selectProjectAction.handleOperation(interaction, OPERATION_IDS.SHOW_PROJECTS, {
        fundingRoundId,
        showUnvoted: true,
      });
...
class SelectProjectAction extends PaginationComponent {
  ...
  private async handleShowProjects(interaction: TrackedInteraction, args: { fundingRoundId: number; showUnvoted: boolean }): Promise<void> {
    ...
    const embed = new EmbedBuilder().setColor('#0099ff').setTitle('Select A Project To Deliberate On')
...
class CommitteeDeliberationVoteAction extends Action {
  ...
  private async handleShowVoteOptions(interaction: TrackedInteraction, args: { projectId: number; fundingRoundId: number }): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Here, you can ${existingVote ? `change your vote (${existingVote.vote ? 'Approved' : 'Rejected'})` : 'vote'} on ${project.name}.`,
      )
      .addFields(
        {
          name: 'Funding Round',
    ...
    const approveButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          OPERATION_IDS.SUBMIT_VOTE,
          'prId',
          projectId.toString(),
          FUNDING_ROUND_ID,
          fundingRoundId.toString(),
          'vote',
          VOTING_OPTION_IDS.APPROVE,
        ),
      )
    ...
    const approveModifiedButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          ...
          fundingRoundId.toString(),
          'vote',
          VOTING_OPTION_IDS.APPROVE_MODIFIED,
        ),
      )
  ...
  private async handleSubmitVote(interaction: TrackedInteraction): Promise<void> {
    ...
    modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(uriInput));
  ...
  private async handleConfirmVote(interaction: TrackedInteraction): Promise<void> {
    ...
    if (voteRaw === VOTING_OPTION_IDS.APPROVE) {
      ...
      description =
        'Your vote to approve the project with modifications has been recorded. You can change it until the end of the deliberation phase.';
