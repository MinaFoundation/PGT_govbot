...
import {
  CreateOrEditFundingRoundAction,
  ManageFundingRoundsScreen,
  ModifyFundingRoundAction,
  SelectTopicAction,
} from '../../admin/screens/ManageFundingRoundsScreen';
...
export class FundingRoundInitScreen extends Screen implements IHomeScreen {
  ...
  protected allSubScreens(): Screen[] {
    return [this.manageFundingRoundScreen];
  }

  protected allActions(): Action[] {
    return [this.voteFundingRoundAction];
  }
  ...
  protected async getResponse(interaction: TrackedInteraction | undefined = undefined, args?: RenderArgs): Promise<any> {
    ...
    const createButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this.manageFundingRoundScreen.createFundingRoundAction,
          CreateOrEditFundingRoundAction.OPERATIONS.START,
    ...
    const editFundingRoundButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this.manageFundingRoundScreen.modifyFundingRoundAction,
    ...
    const voteFundingRoundButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          ...
          VoteFundingRoundAction.OPERATIONS.SHOW_ELIGIBLE_ROUNDS,
          InVotingFundingRoundPaginator.BOOLEAN.ARGUMENTS.FORCE_REPLY,
          InVotingFundingRoundPaginator.BOOLEAN.TRUE,
        ),
      )
      .setLabel('🗳️ Vote On Funding Round')
      .setStyle(ButtonStyle.Primary);

    const row = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(createButton, editFundingRoundButton, voteFundingRoundButton);
...
export class CreateDraftFundingRoundAction extends Action {
  ...
  private async showSetPhaseDates(interaction: TrackedInteraction, fundingRoundId: number): Promise<void> {
    ...
    if (!fundingRound.startAt || !fundingRound.endAt) {
      const setRoundDurationButton = new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            this,
            CreateDraftFundingRoundAction.OPERATIONS.SHOW_SET_PHASE_DATES,
            FUNDING_ROUND_ID_ARG,
    ...
    for (let phaseName of Object.values(CreateDraftFundingRoundAction.NON_ROUND_PHASE_NAMES)) {
      ...
      if (!phase) {
        ...
        embed.addFields({
          name: `${phaseName.charAt(0).toUpperCase() + phaseName.slice(1)} Phase`,
          value: `Start: ${phase.startDate.toUTCString()}\nEnd: ${phase.endDate.toUTCString()}`,
        });
      }
    }
  ...
  private async handleShowSetPhaseDates(interaction: TrackedInteraction): Promise<void> {
    ...
    const phase = CustomIDOracle.getNamedArgument(
      interaction.customId,
      PHASE_ARG,
    )?.toLowerCase() as keyof typeof CreateDraftFundingRoundAction.PHASE_NAMES;
    ...
    const title: string =
      phase == CreateDraftFundingRoundAction.PHASE_NAMES.round
        ? 'Set Round Duration'
        : `Set ${phase.charAt(0).toUpperCase() + phase.slice(1)} Phase Dates`;
    const startDateCustomId: string =
      phase == CreateDraftFundingRoundAction.PHASE_NAMES.round
        ? CreateDraftFundingRoundAction.INPUT_IDS.ROUND_START_DATE
        : CreateDraftFundingRoundAction.INPUT_IDS.START_DATE;
    const endDateCustomId: string =
      phase == CreateDraftFundingRoundAction.PHASE_NAMES.round
        ? CreateDraftFundingRoundAction.INPUT_IDS.ROUND_END_DATE
        : CreateDraftFundingRoundAction.INPUT_IDS.END_DATE;

    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          CreateDraftFundingRoundAction.OPERATIONS.SUBMIT_PHASE_DATES,
          FUNDING_ROUND_ID_ARG,
          fundingRoundId,
          PHASE_ARG,
          phase,
...
export class VoteFundingRoundAction extends PaginationComponent {
  ...
  public readonly inVotingFundingRoundPaginator: InVotingFundingRoundPaginator = new InVotingFundingRoundPaginator(
    this.screen,
    this,
    VoteFundingRoundAction.OPERATIONS.SELECT_ROUND,
    InVotingFundingRoundPaginator.ID,
  );
  ...
  private async handleSelectRound(
    interaction: TrackedInteraction,
    successMessage: string | undefined = undefined,
    errorMesasge: string | undefined = undefined,
  ): Promise<void> {
    const interactionWithValues = InteractionProperties.toInteractionWithValuesOrUndefined(interaction.interaction);
    ...
    if (successMessage) {
      statusEmbed = new EmbedBuilder().setColor('#28a745').setDescription(successMessage);
    } else if (errorMesasge) {
      statusEmbed = new EmbedBuilder().setColor('#dc3545').setDescription(errorMesasge);
    }
    ...
    if (latestVote) {
      ...
      const operation: string = !latestVote.isPass
        ? VoteFundingRoundAction.OPERATIONS.SHOW_APPROVE_MODAL
        : VoteFundingRoundAction.OPERATIONS.SHOW_REJECT_MODAL;
  ...
  private async showVoteModal(interaction: TrackedInteraction, isApprove: boolean): Promise<void> {
    ...
    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          isApprove ? VoteFundingRoundAction.OPERATIONS.SUBMIT_APPROVE : VoteFundingRoundAction.OPERATIONS.SUBMIT_REJECT,
          FUNDING_ROUND_ID_ARG,
          fundingRoundId,
        ),
      )
