...
} from '../../../components/FundingRoundPaginator';
...
export class ManageFundingRoundsScreen extends Screen {
  ...
  constructor(dashboard: Dashboard, screenId: string) {
    ...
    this.crudFRPaginatorAction = new EditFundingRoundPaginator(
      this,
    ...
    this.committeeFRPaginator = new SetCommitteeFundingRoundPaginator(
      ...
      SetFundingRoundCommitteeAction.OPERATIONS.SELECT_FUNDING_ROUND,
      SetCommitteeFundingRoundPaginator.ID,
    );
    this.committeeDeleteFRPaginator = new RemoveCommiteeFundingRoundPaginator(
      this,
      this.removeFundingRoundCommitteeAction,
      RemoveFundingRoundCommitteeAction.OPERATIONS.SELECT_FUNDING_ROUND,
      RemoveCommiteeFundingRoundPaginator.ID,
    );
    this.approveRejectFRPaginator = new ApproveRejectFundingRoundPaginator(
      this,
      this.approveFundingRoundAction,
      ApproveFundingRoundAction.OPERATIONS.SELECT_FUNDING_ROUND,
      ApproveRejectFundingRoundPaginator.ID,
    );
  }
  ...
  protected async getResponse(interaction: TrackedInteraction, args?: RenderArgs): Promise<any> {
    ...
    const row1 = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(createButton, modifyButton);

    const row2 = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(addCommitteeButton, removeCommitteeButton);

    const row3 = new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(approveButton);
...
export class CreateOrEditFundingRoundAction extends Action {
  ...
  private async handleStart(interaction: TrackedInteraction): Promise<void> {
    const isForceReply: boolean = ArgumentOracle.isArgumentEquals(
      interaction,
      CreateOrEditFundingRoundAction.BOOLEANS.ARGUMENTS.FORCE_REPLY,
  ...
  private formatStringForRound(fundingRound: FundingRound): string {
    return `✅\nStart: ${fundingRound.startAt.toUTCString()}\nEnd: ${fundingRound.endAt.toUTCString()}\nVoting Open Until: ${fundingRound.votingOpenUntil.toUTCString()}\nEpoch: ${
      fundingRound.stakingLedgerEpoch
    }`;
  }
  ...
  private async handleShowProgress(interaction: TrackedInteraction): Promise<void> {
    ...
    const onlyShowPhases: boolean = ArgumentOracle.isArgumentEquals(
      interaction,
      CreateOrEditFundingRoundAction.BOOLEANS.ARGUMENTS.ONLY_SHOW_PHASES,
      CreateOrEditFundingRoundAction.BOOLEANS.TRUE_VALUE,
    );
    ...
    const progress: string = (await fundingRound.isReady())
      ? '✅ READY\n\nThe Funding Round information is complete & valid.'
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Topic', value: fundingRound.topicId ? `✅\n${topic.name}` : '❌', inline: true },
        {
          ...
          value:
            fundingRound.name && fundingRound.description && fundingRound.budget && fundingRound.stakingLedgerEpoch
              ? `✅\nName: ${fundingRound.name}\nDescription: ${fundingRound.description}\nBudget: ${fundingRound.budget}\Epoch For Voting: ${fundingRound.stakingLedgerEpoch}`
              : '❌',
          inline: true,
        },
        {
          name: 'Funding Round Dates',
          value: fundingRound.startAt && fundingRound.endAt && fundingRound.votingOpenUntil ? this.formatStringForRound(fundingRound) : '❌',
          inline: true,
        },
    ...
    const manageDatesCustomId: string = CustomIDOracle.addArgumentsToAction(
      this,
      CreateOrEditFundingRoundAction.OPERATIONS.SHOW_PROGRESS,
    ...
    const initScreenButtons = [
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            (this.screen as ManageFundingRoundsScreen).selectTopicAction,
            SelectTopicAction.OPERATIONS.SHOW_TOPICS,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId,
          ),
        )
      ...
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            (this.screen as ManageFundingRoundsScreen).selectForumChannelAction,
            SelectForumChannelAction.OPERATIONS.SHOW_CHANNELS,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId,
          ),
        )
      ...
      new ButtonBuilder().setCustomId(manageDatesCustomId).setLabel('Manage Phases & Dates').setStyle(ButtonStyle.Primary),

      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            (this.screen as ManageFundingRoundsScreen).coreInformationAction,
            CoreInformationAction.OPERATIONS.SHOW_FORM,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId,
          ),
        )
    ...
    const phaseDatesOnlyButtons = [
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            (this.screen as ManageFundingRoundsScreen).setPhaseAction,
            SetPhaseAction.OPERATIONS.SHOW_FORM,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId,
            SetPhaseAction.ARGUMENTS.PHASE,
            FundingRoundMI.PHASES.ROUND,
          ),
        )
      ...
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            ...
            fundingRoundId,
            SetPhaseAction.ARGUMENTS.PHASE,
            FundingRoundMI.PHASES.CONSIDERATION,
          ),
        )
      ...
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            ...
            fundingRoundId,
            SetPhaseAction.ARGUMENTS.PHASE,
            FundingRoundMI.PHASES.DELIBERATION,
          ),
        )
...
export class SelectTopicAction extends PaginationComponent {
  ...
  public async handlePagination(interaction: TrackedInteraction, isForceReply: boolean = false): Promise<void> {
    ...
    const description: string = fundingRoundId
      ? 'Select a topic to update the Funding Round topic.'
      : 'Select a topic under which the new Funding Round will be created.';
    const customId: string = fundingRoundId
      ? CustomIDOracle.addArgumentsToAction(
          this,
          SelectTopicAction.OPERATIONS.UPDATE_TOPIC,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId,
        )
      : CustomIDOracle.addArgumentsToAction(this, SelectTopicAction.OPERATIONS.SELECT_TOPIC);

    const infoEmbed: EmbedBuilder = new EmbedBuilder().setTitle('Select a Topic').setDescription(description);
...
export class CoreInformationAction extends Action {
  ...
  private async handleShowForm(interaction: TrackedInteraction): Promise<void> {
    ...
    const customId: string = fundingRoundId
      ? CustomIDOracle.addArgumentsToAction(
          this,
          CoreInformationAction.OPERATIONS.SUBMIT_FORM,
          CoreInformationAction.INPUT_IDS.TOPIC,
          parsedTopicId,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId,
        )
      : CustomIDOracle.addArgumentsToAction(this, CoreInformationAction.OPERATIONS.SUBMIT_FORM, CoreInformationAction.INPUT_IDS.TOPIC, parsedTopicId);
    const modal = new ModalBuilder().setCustomId(customId).setTitle('Funding Round Core Information');
...
export class SetPhaseAction extends Action {
  ...
  private async handleShowForm(interaction: TrackedInteraction): Promise<void> {
    ...
    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          SetPhaseAction.OPERATIONS.SUBMIT_FORM,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId,
          ArgumentOracle.COMMON_ARGS.PHASE,
          phase,
        ),
      )
    ...
    if (phase === FundingRoundMI.PHASES.ROUND) {
      ...
      modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(votingOpenUntilInput));
    }
  ...
  private async handleSubmitForm(interaction: TrackedInteraction): Promise<void> {
    ...
    if (parsedPhase == FundingRoundMI.PHASES.ROUND) {
      ...
      const fundingRound: FundingRound = await FundingRoundLogic.updateFundingRoundPhase(
        parseInt(fundingRoundId),
        parsedPhase,
        stakingLedgerEpoch,
        startDate,
        endDate,
  ...
  public async handlePagination(interaction: TrackedInteraction): Promise<void> {
    ...
    const selectMenu = new StringSelectMenuBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          SelectForumChannelAction.OPERATIONS.SELECT_CHANNEL,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId,
        ),
      )
  ...
  private async showCommitteeMemberSelection(interaction: TrackedInteraction, fundingRoundId: number): Promise<void> {
    ...
    const userSelect = new UserSelectMenuBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          SetFundingRoundCommitteeAction.OPERATIONS.SELECT_COMMITTEE_MEMBERS,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
  ...
  private async handleConfirmApproval(interaction: TrackedInteraction): Promise<void> {
    ...
    const approveButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          ApproveFundingRoundAction.OPERATIONS.EXECUTE_APPROVAL,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
    ...
    const rejectButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          ApproveFundingRoundAction.OPERATIONS.EXECUTE_REJECTION,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
...
export class RemoveFundingRoundCommitteeAction extends PaginationComponent {
  ...
  private async showMemberRemovalSelection(interaction: TrackedInteraction, fundingRoundId: number): Promise<void> {
    ...
    const userSelect = new UserSelectMenuBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          RemoveFundingRoundCommitteeAction.OPERATIONS.SELECT_MEMBERS_TO_REMOVE,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
    ...
    const removeAllButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          RemoveFundingRoundCommitteeAction.OPERATIONS.REMOVE_ALL_MEMBERS,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
  ...
  private async showCommitteeStatus(interaction: TrackedInteraction, fundingRoundId: number, numRemovedRecords: number): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Members removed: ${numRemovedRecords}. ${
          isComplete ? 'The committee meets all requirements.' : 'The committee does not meet all requirements.'
        }`,
      );
...
export class SelectFundingRoundToEditAction extends PaginationComponent {
  ...
  private async handleShowFundingRounds(interaction: TrackedInteraction): Promise<void> {
    ...
    const customId: string = CustomIDOracle.addArgumentsToAction(
      (this.screen as ManageFundingRoundsScreen).createFundingRoundAction,
      CreateOrEditFundingRoundAction.OPERATIONS.SHOW_PROGRESS,
    );
  ...
  private async handleShowEditOptions(interaction: TrackedInteraction): Promise<void> {
    ...
    const editInfoButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          EditFundingRoundTypeSelectionAction.OPERATIONS.SELECT_EDIT_TYPE,
          'type',
          EditFundingRoundTypeSelectionAction.EDIT_TYPES.INFO,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
    ...
    const editPhasesButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          ...
          EditFundingRoundTypeSelectionAction.OPERATIONS.SELECT_EDIT_TYPE,
          'type',
          EditFundingRoundTypeSelectionAction.EDIT_TYPES.PHASES,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
    ...
    const editTopicButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          ...
          EditFundingRoundTypeSelectionAction.OPERATIONS.SELECT_EDIT_TYPE,
          'type',
          EditFundingRoundTypeSelectionAction.EDIT_TYPES.TOPIC,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(editInfoButton, editPhasesButton, editTopicButton);
  ...
  private async handleSelectEditType(interaction: TrackedInteraction): Promise<void> {
    ...
    switch (editType) {
      case EditFundingRoundTypeSelectionAction.EDIT_TYPES.INFO:
        await screen.editFundingRoundInformationAction.handleOperation(interaction, EditFundingRoundInformationAction.OPERATIONS.SHOW_EDIT_FORM);
        break;
      case EditFundingRoundTypeSelectionAction.EDIT_TYPES.PHASES:
        await screen.editFundingRoundPhasesAction.handleOperation(interaction, EditFundingRoundPhasesAction.OPERATIONS.SHOW_PHASE_OPTIONS);
        break;
      case EditFundingRoundTypeSelectionAction.EDIT_TYPES.TOPIC:
        await screen.editFundingRoundTopicAction.handleOperation(interaction, EditFundingRoundTopicAction.OPERATIONS.SHOW_TOPIC_SELECTION);
        break;
      default:
  ...
  private async handleShowEditForm(interaction: TrackedInteraction): Promise<void> {
    ...
    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          EditFundingRoundInformationAction.OPERATIONS.SUBMIT_EDIT,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
...
export class EditFundingRoundPhasesAction extends Action {
  ...
  private async handleShowPhaseOptions(interaction: TrackedInteraction): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        name: 'Funding Round Duration',
        value: `Start: ${fundingRound.startAt?.toISOString() || 'Not set'}\nEnd: ${fundingRound.endAt?.toISOString() || 'Not set'}`,
        inline: false,
      });
    ...
    for (const phase of phases) {
      embed.addFields({
        name: `${phase.phase.charAt(0).toUpperCase() + phase.phase.slice(1)} Phase`,
        value: `Start: ${phase.startDate.toISOString()}\nEnd: ${phase.endDate.toISOString()}`,
        inline: false,
      });
    }

    embed.addFields({ name: 'Staking Ledger Epoch', value: fundingRound.stakingLedgerEpoch.toString(), inline: false });
    ...
    const buttons = Object.values(EditFundingRoundPhasesAction.PHASE_NAMES).map((phase) =>
      new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            this,
            EditFundingRoundPhasesAction.OPERATIONS.EDIT_PHASE,
            ArgumentOracle.COMMON_ARGS.PHASE,
            phase,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId.toString(),
    ...
    const backToEditFundingRoundBtn: ButtonBuilder = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          (this.screen as ManageFundingRoundsScreen).selectFundingRoundToEditAction,
          SelectFundingRoundToEditAction.OPERATIONS.SELECT_FUNDING_ROUND,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
        ),
      )
  ...
  private async handleEditPhase(interaction: TrackedInteraction): Promise<void> {
    ...
    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(
          this,
          EditFundingRoundPhasesAction.OPERATIONS.SUBMIT_PHASE_EDIT,
          ArgumentOracle.COMMON_ARGS.PHASE,
          phase,
          ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
          fundingRoundId.toString(),
  ...
  private async handleSubmitPhaseEdit(interaction: TrackedInteraction): Promise<void> {
    ...
    const stakingLedgerEpochNum: number = parseInt(
      modalInteraction.fields.getTextInputValue(EditFundingRoundPhasesAction.INPUT_IDS.STAKING_LEDGER_EPOCH),
    );
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: 'Funding Round Duration',
            value: `Start: ${updatedFundingRound?.startAt?.toISOString() || 'Not set'}\nEnd: ${
              updatedFundingRound?.endAt?.toISOString() || 'Not set'
            }`,
            inline: false,
          },
      ...
      for (const updatedPhase of updatedPhases) {
        embed.addFields({
          name: `${updatedPhase.phase.charAt(0).toUpperCase() + updatedPhase.phase.slice(1)} Phase`,
          value: `Start: ${updatedPhase.startDate.toISOString()}\nEnd: ${updatedPhase.endDate.toISOString()}`,
          inline: false,
        });
      }
  ...
  private async handleSelectTopic(interaction: TrackedInteraction): Promise<void> {
    ...
    try {
      ...
      const backButton = new ButtonBuilder()
        .setCustomId(
          CustomIDOracle.addArgumentsToAction(
            (this.screen as ManageFundingRoundsScreen).editFundingRoundTypeSelectionAction,
            EditFundingRoundTypeSelectionAction.OPERATIONS.SHOW_EDIT_OPTIONS,
            ArgumentOracle.COMMON_ARGS.FUNDING_ROUND_ID,
            fundingRoundId.toString(),
          ),
