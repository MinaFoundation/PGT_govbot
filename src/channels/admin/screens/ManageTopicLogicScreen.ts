...
import { TopicCommitteeWithSMEGroup } from '../../../types';
...
class SelectTopicAction extends Action {
  ...
  private async handleSelectTopic(interaction: TrackedInteraction): Promise<void> {
    ...
    if (allAllowedSMEGroups.length > 0) {
      embed.addFields({ name: 'Allowed Reviewer Groups', value: allAllowedSMEGroups.join(', ') });
    } else {
      embed.addFields({ name: 'Allowed Reviewer Groups', value: 'All groups allowed' });
    }

    // Add committee information
    if (topicDetails.committees.length > 0) {
      const committeeInfos: string[] = await Promise.all(
        topicDetails.committees.map(async (committee) => {
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(editButton, manageCommitteesButton, removeButton);
  ...
  getComponent(topics: Topic[]): StringSelectMenuBuilder | ButtonBuilder {
    if (topics.length === 0) {
      return new ButtonBuilder().setCustomId('no_topics').setLabel('🗑️ No Topics available').setStyle(ButtonStyle.Secondary).setDisabled(true);
    } else {
...
class AddTopicAction extends Action {
  ...
  protected async handleAddTopicFormDisplay(interaction: TrackedInteraction): Promise<void> {
    ...
    const modal = new ModalBuilder().setCustomId(customId).setTitle('Add New Topic');
  ...
  getComponent(): ButtonBuilder {
    ...
    return new ButtonBuilder().setCustomId(customIdWithOperation).setLabel('Add New Topic').setStyle(ButtonStyle.Success);
  }
}
...
export class ManageTopicCommitteesAction extends Action {
  ...
  private async handleEditCommitteeForm(interaction: TrackedInteraction): Promise<void> {
    ...
    const modal = new ModalBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(this, ManageTopicCommitteesAction.Operations.EDIT_COMMITTEE_SUBMIT, 'committeeId', committeeId),
      )
    ...
    modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(numUsersInput));
  ...
  private async handleRemoveCommitteeConfirm(interaction: TrackedInteraction): Promise<void> {
    ...
    const confirmButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(this, ManageTopicCommitteesAction.Operations.REMOVE_COMMITTEE_EXECUTE, 'committeeId', committeeId),
      )
    ...
    const cancelButton = new ButtonBuilder()
      .setCustomId(
        CustomIDOracle.addArgumentsToAction(this, ManageTopicCommitteesAction.Operations.VIEW_COMMITTEES, 'topicId', committee.topicId.toString()),
      )
      .setLabel('Cancel')
      .setStyle(ButtonStyle.Secondary);
  ...
  private getDetailedErrorMessage(error: any): string {
    if (error instanceof Error) {
      if ('name' in error && error.name === 'SequelizeUniqueConstraintError') {
        return 'A committee for this Reviewer group already exists for this topic.';
      } else if ('name' in error && error.name === 'SequelizeForeignKeyConstraintError') {
        return 'The specified Reviewer group does not exist.';
      } else {
    ...
    committees.forEach((committee) => {
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
          .setCustomId(
            CustomIDOracle.addArgumentsToAction(
              (this.screen as ManageTopicsScreen).manageTopicCommitteesAction,
              ManageTopicCommitteesAction.Operations.EDIT_COMMITTEE_FORM,
              'committeeId',
        ...
        new ButtonBuilder()
          .setCustomId(
            CustomIDOracle.addArgumentsToAction(
              (this.screen as ManageTopicsScreen).manageTopicCommitteesAction,
              ManageTopicCommitteesAction.Operations.REMOVE_COMMITTEE_CONFIRM,
              'committeeId',
              committee.id.toString(),
            ),
          )
